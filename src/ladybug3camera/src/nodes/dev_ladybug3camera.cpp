...
#define CAM_EXCEPT(except, msg)					\
  {								\
    char buf[100];						\
    snprintf(buf, 100, "[Ladybug3Camera::%s]: " msg, __FUNCTION__); \
    throw except(buf);						\
  }
...
#define CAM_EXCEPT_ARGS(except, msg, ...)				\
  {									\
    char buf[100];							\
    snprintf(buf, 100, "[Ladybug3Camera::%s]: " msg, __FUNCTION__, __VA_ARGS__); \
    throw except(buf);							\
  }


using namespace ladybug3camera;
...
void Ladybug3Camera::findBayerPattern(const char* bayer)
{
...
bool Ladybug3Camera::findBayerMethod(const char* method)
{
 ...
 */
int Ladybug3Camera::open(ladybug3camera::Ladybug3CameraConfig &newconfig)
{
  //////////////////////////////////////////////////////////////
  ...
  if (d == NULL)
    {
      CAM_EXCEPT(ladybug3camera::Exception,
  ...
  if (!camera_)
    {
      if (strcmp(guid,"")==0)
        ...
        {
          CAM_EXCEPT_ARGS(ladybug3camera::Exception,
  ...
  if (DC1394_SUCCESS != dc1394_video_set_mode(camera_, videoMode_))
    {
      SafeCleanup();
      CAM_EXCEPT(ladybug3camera::Exception, "Failed to set video mode");
      return -1;
    }
  ...
  if (dc1394_is_video_mode_scalable(videoMode_) == DC1394_TRUE)
    {
      ...
      if (!format7_.start(camera_, videoMode_, newconfig))
        {
          SafeCleanup();
          CAM_EXCEPT(ladybug3camera::Exception, "Format7 start failed");
          return -1;
        }
    }
  else
    {
      ...
      if (!Modes::setFrameRate(camera_, videoMode_, newconfig.frame_rate))
        {
          SafeCleanup();
          CAM_EXCEPT(ladybug3camera::Exception, "Failed to set frame rate");
          return -1;
        }
    }
  ...
  if (DC1394_SUCCESS != dc1394_video_set_transmission(camera_, DC1394_ON))
    {
      SafeCleanup();
      CAM_EXCEPT(ladybug3camera::Exception, "Failed to start device!");
      return -1;
    }
...
void Ladybug3Camera::SafeCleanup()
{
  if (camera_)
    {
      format7_.stop();
